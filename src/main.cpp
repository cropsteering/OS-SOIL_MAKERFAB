...
static bool trigger_once = false; // Button anti-spam
static bool lorawan_joined = false;
...
void loop()
{
    ...
    if (count > SLEEP_CYCLE) //(7+1) x 8S  450
    {
        ...
        if(lorawan_joined)
        {
            // Already joined, send data
            log_out("[Sending LoRaWAN Data]");
            send_lorawan();
        } else {
            // Not joined, retry, send data
            log_out("[Retrying LoRaWAN OTAA]");
            while(lorawan_join())
            {
                send_lorawan();
                break;
...
int lorawan_join()
{
    ...
    if(sendData_keyword("AT+CJOIN=1,0,8,8", 30000, "Joined"))
    {
        lorawan_joined = true;
        return 1;
    } else {
